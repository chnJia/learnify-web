...
const courses: Course[] = [
    ...
    },
    {
        ...
        description: "An introductory course for senior high students on Python fundamentals, including variables, data types, and control structures.",
    ...
    },
    {
        ...
        description: "A complete course to excel in IELTS, covering Listening, Reading, Writing, and Speaking with tips and strategies from a 9.0 scorer.",
        teacher: "Belinda Wati",
        rating: 4.3,
        price: 280000,
        lessons: [
            {
                ...
                description: "Learn the format, scoring system, and expectations for the IELTS test to develop a strong foundation."
            },
            {
                session: "Session 2",
                topic: "Listening Skills and Practice",
                description: "Master strategies for the Listening section, with practice exercises to improve comprehension and accuracy."
            },
            {
                session: "Session 3",
                topic: "Reading Comprehension Strategies",
                description: "Develop techniques to effectively manage time and answer questions in the Reading section."
            },
            {
                session: "Session 4",
                topic: "Writing Task 1: Data Interpretation",
                description: "Learn how to analyze and describe data, charts, and graphs clearly and accurately."
            },
            {
                session: "Session 5",
                topic: "Writing Task 2: Essay Writing",
                description: "Understand how to write structured essays with clear arguments and examples to score high in Task 2."
            },
            {
                session: "Session 6",
                topic: "Speaking Skills and Mock Tests",
                description: "Practice speaking fluently and confidently for the IELTS Speaking test, with tips on common topics."
            },
            {
                session: "Session 7",
                topic: "Test Day Preparation and Tips",
                description: "Learn strategies to manage time, reduce stress, and maximize your performance on test day."
            },
        ],
        reviews: [
            { name: "John", comment: "Great course!", rating: 4 },
...
const CourseDetail = () => {
    ...
    return (
        <div className='flex flex-col px-4'>
            <div className="mt-3 bg-light-blue py-3 px-2 relative flex flex-col md:flex-row">
                <div className="w-full md:w-1/2 md:pr-4 sm:pr-0 mb-4 md:mb-0">
                    <img
                    src={course.image}
                    alt={course.title}
                ...
                <div className="w-full md:w-1/2 pl-4">
                    ...
                    <div className="flex items-center space-x-1 text-yellow-500 ml-2">
                        {Array(fullStars).fill(<FaStar className="text-xs sm:text-sm" />)}
                        {halfStar > 0 && <FaStarHalfAlt className="text-xs sm:text-sm" />}
                        {Array(emptyStars).fill(<FaRegStar className="text-xs sm:text-sm" />)}
                    </div>
                    </div>
                    ...
                    <span className="text-md font-bold text-zinc-800">
                        <button
                        className="bg-dark-moderate-blue text-white py-2 px-4 sm:py-2 sm:px-5 rounded-full hover:bg-azure"
                        onClick={handleCourseSubs}
                        >
                        Subscribe Course
                        </button>
                    </span>
                ...
                <div className='mt-4'>
                    {activeTab === 'lessons' && (
                        <div className='bg-white shadow rounded-lg p-6 text-zinc-800'>
                            <h3 className='text-xl sm:text-2xl font-bold mb-4 text-moderate-blue'>Course Overview</h3>
                            <p className='mb-4 text-sm sm:text-base text-gray-700'>{course.description}</p>
                            <h4 className='text-lg sm:text-xl font-semibold text-moderate-blue mb-3'>Lessons Outline</h4>
                            <ul className='space-y-4'>
                                {course.lessons.map((lesson: Lesson, index: number) => (
                                    <li key={index} className='p-4 border rounded-lg bg-gray-50 hover:bg-gray-100 transition'>
                                        <h5 className='text-sm sm:text-lg font-semibold text-azure'>{lesson.session}: {lesson.topic}</h5>
                                        <p className='text-sm sm:text-base text-gray-600 mt-2'>{lesson.description}</p>
                                    </li>
                                ))}
                            </ul>
                        </div>
                    ...
                    {activeTab === 'review' && (
                        <div className='bg-white shadow rounded-lg p-6 text-zinc-800'>
                            <h3 className='text-xl sm:text-2xl font-bold mb-4 text-moderate-blue'>Reviews</h3>
                                    ...
                                    <li key={index} className='p-4 border rounded-lg bg-gray-50 hover:bg-gray-100 transition'>
                                        <div className='flex items-center mb-2'>
                                            ...
                                            <div className='flex flex-col'>
                                                <span className='font-semibold text-sm sm:text-lg text-zinc-800'>{review.name}</span>
                                                ...
                                                <div className='flex items-center text-yellow-500'>
                                                    ...
                                                    <span className='ml-2 text-xs sm:text-sm text-gray-500'>2h ago</span>
                                                </div>
                                            </div>
                                        </div>

                                        <p className='text-sm sm:text-base text-gray-600 mt-2 line-clamp-4'>
                                            {review.comment}
                                        </p>
                                    </li>
                                ))}
